...
Сводная таблица имён переменных окружения и откуда брать значения (Google Console и т.д.): **`README.md`** (раздел «Переменные окружения») и **`.env.example`** в корне репозитория.
