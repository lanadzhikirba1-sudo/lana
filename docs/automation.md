...
- планирует и отправляет **payment reminder** в Telegram через очередь `notification_jobs`.
...
`Backend worker → Telegram Bot API` (отправка reminder)
...
| `Authorization: Bearer <token>` **или** `X-Internal-Token: <token>` | для `/api/v1/bot/*` | общий секрет между конструктором и backend |

Рекомендация по реализации: принимать оба варианта и маппить на одну проверку.
...
- `calendar_connections` — OAuth-токены, `calendar_id`, `sync_token`, поля push-watch
...
Периодически (например каждые 1–5 минут):

1. Выбрать из `notification_jobs` записи: `status = pending` и `scheduled_for <= now()` с лимитом и `FOR UPDATE SKIP LOCKED` (рекомендация).
2. Отправить сообщение в Telegram `sendMessage` в `target_chat_id`.
3. Успех: `status = sent`, `sent_at = now()`, `last_error = null`.
4. Ошибка: `status = failed`, `last_error` заполнить; политика ретраев — в конфиге (например 3 попытки с backoff).
...
**Пересчёт reminder:** при изменении `therapists.timezone` выполнить пересчёт связанных `notification_jobs` по правилам раздела 7 (как и при любом обновлении настроек терапевта, влияющих на расписание уведомлений).
...
Поведение: обмен кода на токены, сохранение в `calendar_connections`, запуск initial sync.
...
Доступ: только internal token / сеть.
...
|---------|--------|----------|
| `reminder_worker` | 1–5 мин | отправка due `notification_jobs` |
...
| `token_refresh` | по необходимости | обновление access token перед API вызовами |
...
- логировать отправку reminder: `notification_job_id`, `target_chat_id`, ответ Telegram.
...
- [ ] Идемпотентность confirm-payment и register-chat
