...
- планирует **payment reminder** через очередь `notification_jobs`; отправку сообщения в Telegram по тексту сценария выполняет **конструктор ботов**, backend фиксирует факт отправки через API (см. §7.6 и §9.12–9.13).
...
В v1 **текст сообщения и вызов Telegram API** выполняет конструктор ботов (сценарий, BotHelp и т.п.). Backend отвечает за **планирование** (`scheduled_for`, `pending`/`cancelled`) и за **фиксацию факта отправки или сбоя** по вызовам из конструктора (см. §9.12–9.13).
...
Альтернатива «всё в backend» (воркер сам делает `sendMessage`) допустима как упрощённая реализация, но **тогда не выполняется** продуктовое требование v1, что копирайт и сценарий текста живут в конструкторе.
